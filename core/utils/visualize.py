...
def get_color_pallete(npimg, tg, dataset='substrate'):
    # --- make inputs numpy ---
    if torch.is_tensor(npimg):
        npimg = npimg.detach().cpu().numpy()
    if torch.is_tensor(tg):
        tg = tg.detach().cpu().numpy()

    # ensure integer types
    npimg = npimg.astype(np.int32, copy=False)
    tg    = tg.astype(np.int32, copy=False)

    if dataset in ('pascal_voc', 'pascal_aug'):
        npimg[npimg == -1] = 255

    if len(tg.shape) == 3 and tg.shape[0] == 1:
        tg = np.squeeze(tg, axis=0)
    ...
    elif dataset == 'substrate':
        out_img = Image.fromarray(npimg.astype('uint8'))
        raw_msk = Image.fromarray(npimg.astype('uint8'))

        out_msk = Image.fromarray(tg.astype('uint8'))
        gt_msk  = Image.fromarray(tg.astype('uint8'))

        out_img.putpalette(substratepallete)
        out_msk.putpalette(substratepallete)
        return out_img, out_msk, raw_msk, gt_msk
...
substratepallete = [
     ...
     #255, 0, 0,
     0, 255, 0,
     0, 0, 255, #temp change, for shadow only case
