...
def make_data_sampler(dataset, shuffle, distributed):
    ...
    if not shuffle:
        #sampler = data.sampler.RandomSampler(dataset)
        return data.sampler.SequentialSampler(dataset)

    
    
    valid_indices = [i for i, cls in enumerate(dataset.get_class_labels()) if cls != 0]
    #print(valid_indices)
    
    # Weights for remaining classes
    class_weights = {
        1: 10,    # Shadow
        2: 0.4   # Others (25× more frequent)
    }
    
    # Create weights only for valid indices
    sample_weights = torch.tensor([class_weights[dataset.get_class_labels()[i]] 
                                 for i in valid_indices])
    
    # Create subset sampler
    return WeightedRandomSampler(
        weights=sample_weights,
        num_samples=len(valid_indices),  # Only sample from valid indices
        replacement=True
    )
