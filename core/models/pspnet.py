...
class PSPNet(SegBaseModel):
    ...
    def __init__(self, nclass, backbone='resnet50', aux=False, pretrained_base=True, **kwargs):
        ...
        if backbone.startswith('resnet'):
            self.base = globals()[backbone](pretrained=pretrained_base, norm_layer=kwargs.get('norm_layer'))
...
def get_psp(dataset='pascal_voc', backbone='resnet50', pretrained=False, root='../Trained_weight',
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('psp_resnet50_substrate_pspnet_best_substrateshadow_train2', root=root),
                              map_location=device))
    return model
