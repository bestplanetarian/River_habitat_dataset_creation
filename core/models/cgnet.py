...
class CGNet(nn.Module):
    ...
    def __init__(self, nclass, backbone='', aux=False, jpu=False, pretrained_base=True, M=3, N=21, **kwargs):
        ...
        # stage 1
        self.stage1_0 = _ConvBNPReLU(1, 32, 3, 2, 1, **kwargs)
        ...
        self.bn_prelu1 = _BNPReLU(32 + 1, **kwargs)

        # stage 2
        self.stage2_0 = ContextGuidedBlock(32 + 1, 64, dilation=2, reduction=8, down=True, residual=False, **kwargs)
        self.stage2 = nn.ModuleList()
        for i in range(0, M - 1):
            self.stage2.append(ContextGuidedBlock(64, 64, dilation=2, reduction=8, **kwargs))
        self.bn_prelu2 = _BNPReLU(128 + 1, **kwargs)

        # stage 3
        self.stage3_0 = ContextGuidedBlock(128 + 1, 128, dilation=4, reduction=16, down=True, residual=False, **kwargs)
...
def get_cgnet(dataset='citys', backbone='', pretrained=False, root='~/.torch/models', pretrained_base=True, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('cgnet_resnet50_substrate_weight_wholeset_best_cgnet1_RGB', root=root),
                              map_location=device))
    return model
