...
def resnet50_v1s(pretrained=False, root='~/.torch/models', **kwargs):
    ...
    if pretrained:
        ...
        weights = torch.load(get_resnet_file('resnet50', root=root))
        weights['conv1.0.weight'] = weights['conv1.0.weight'].sum(1, keepdim=True)
        model.load_state_dict(weights, strict=False)
        #weights = model.state_dict()
        #weights['conv1.0.weight'] = weights['conv1.0.weight'].sum(1, keepdim=True)
        #weights['conv1.0.weight'] = weights['conv1.0.weight'][:,:1,:,:]
        #model.load_state_dict(weights, strict=False)
