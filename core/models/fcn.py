...
class FCN32s(nn.Module):
    ...
    def __init__(self, nclass, backbone='vgg16', aux=False, pretrained_base=True,
        ...
        if backbone == 'vgg16':
            ...
            self.pretrained[0] = nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1, bias=False)
        else:
...
def get_fcn32s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='../Trained_weight',
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('fcn32s_vgg16_substrate_fcn32_best_substrateshadow_train2', root=root),
                              map_location=device))
    return model
...
def get_fcn8s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='../Trained_weight',
              pretrained_base=True, **kwargs):
    acronyms = {
        'pascal_voc': 'pascal_voc',
        'pascal_aug': 'pascal_aug',
    ...
    if pretrained:
        from .model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(torch.load(get_model_file('fcn8s_vgg16_substrate_fcn8_best_substrateshadow_train2', root=root),
                              map_location=device))
    return model
